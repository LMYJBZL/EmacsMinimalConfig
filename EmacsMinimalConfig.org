...
#+AUTHOR: Baojie
#+DATE: Time-stamp: <2026-07-06 19:43:34 baojie>
:PROPERTIES:
#+STARTUP: content
...
(use-package all-the-icons
  ...
  :ensure t
  :config
  ;; The upstream fallback uses the legacy FontAwesome family.  Use the
  ;; installed Octicons family instead, so unknown files keep the same glyph
  ;; when Vertico adds or removes its current-candidate face.
  (setq all-the-icons-default-file-icon
        '(all-the-icons-octicon "file-text"
          :v-adjust 0.0 :face all-the-icons-dsilver))
  ;; `messages-buffer-mode' also defaults to the unavailable legacy
  ;; FontAwesome family, independently of `all-the-icons-default-file-icon'.
...
(use-package all-the-icons-completion
  ...
  ;; Reserve a two-column icon slot.  Padding on the left makes every icon
  ;; share the same right edge; the final space separates it from the label.
