...
(use-package consult
  ...
  (defun my/recentf-display-name (file-path duplicate-table)
    "根据 DUPLICATE-TABLE 为 FILE-PATH 生成简洁且可区分的显示名。"
    (let* ((base-name (file-name-nondirectory file-path))
      ...
      (if (> (gethash base-name duplicate-table 0) 1)
          (format "%s    <%s>" base-name (file-name-nondirectory dir-name))
        base-name)))
    ...
    (let ((ht (consult--buffer-file-hash))
      ...
      (dolist (file recentf-list)
        ...
        (let ((file-path (if (stringp file) file (cdr file))))
          ...
          ;; 跳过已在缓冲区的文件
          (unless (or (null file-path) (gethash file-path ht))
            (push (propertize (my/recentf-display-name file-path duplicate-table)
