...
#+begin_src emacs-lisp
(defvar my/package-refresh-on-missing-use-package nil
  "Non-nil means refresh package archives at startup if `use-package' is missing.")
...
(when (and (bound-and-true-p minimal-emacs-package-initialize-and-refresh)
  ...
  (unless (package-installed-p 'use-package)
    (when (and my/package-refresh-on-missing-use-package
               (not package-archive-contents))
      (package-refresh-contents))
    (if package-archive-contents
        (package-install 'use-package)
      (user-error
       "use-package is missing; run M-x package-refresh-contents, then restart Emacs")))
  (require 'use-package))
#+end_src
...
(defun my/tex-font-help-buffer-p (buffer-name _action)
  "Return non-nil when BUFFER-NAME is the AUCTeX font help buffer."
  (when-let ((buffer (get-buffer buffer-name)))
    (with-current-buffer buffer
      (save-excursion
        (goto-char (point-min))
        (looking-at-p "Font list:")))))
...
(defun my/tex-help-fixed-width ()
  (when-let ((help-win (get-buffer-window "*TeX Help*")))
    (let ((current (window-total-width help-win))
          (target 76))
      (when (> (abs (- current target)) 2)
        (window-resize help-win (- target current) t)))))

(add-hook 'window-configuration-change-hook
          #'my/tex-help-fixed-width)
...
(use-package vertico
  ...
  ;;    (window-width . 0.33)))
  :config
  (defun my/vertico-crm-indicator (args)
    "Prefix the prompt in `completing-read-multiple' with its separator."
    (cons (format "[CRM%s] %s"
  ...
  (when (< emacs-major-version 31)
    (unless (advice-member-p #'my/vertico-crm-indicator
                             #'completing-read-multiple)
      (advice-add #'completing-read-multiple
                  :filter-args
                  #'my/vertico-crm-indicator)))
...
(use-package rime
  ...
  (defun my/enable-rime-in-writing-modes ()
    "在日常写作模式中自动启用 Rime。"
    (unless (or (equal current-input-method "rime")
                (minibufferp)
                (derived-mode-p 'special-mode 'help-mode 'messages-buffer-mode
                                'log-edit-mode 'git-commit-mode))
      (activate-input-method "rime")))

  (dolist (hook '(text-mode-hook org-mode-hook LaTeX-mode-hook))
    (add-hook hook #'my/enable-rime-in-writing-modes))
  ...
  (defun my/set-all-frames-cursor-color (color)
    ...
    (dolist (frame (frame-list))
      ;; 自动中英文判断会在每条命令后运行；只有颜色实际变化时才写 frame。
...
(use-package consult
  ...
  (defun my/recentf-normalize-path (file)
    "将 recentf 中的 FILE 规范化为绝对路径字符串。"
    (let ((file-path (if (stringp file) file (cdr file))))
      (when (and (stringp file-path) (not (string-empty-p file-path)))
        (expand-file-name file-path))))

  (defun my/recentf-latex-generated-p (file-path)
    "Return non-nil when FILE-PATH is a generated LaTeX cache/output file."
    (string-match-p "/\\.LaTeX\\(?:Auto\\|Out\\)/" file-path))

  (defun my/recentf-unique-directory-suffix (file-path files)
    "为 FILE-PATH 生成相对 FILES 中同名文件的最短唯一路径后缀。"
    (let* ((target-dir
            (split-string
             (directory-file-name
              (file-name-directory (abbreviate-file-name file-path)))
             "/" t))
           (same-name-files
            (seq-filter
             (lambda (other)
               (string= (file-name-nondirectory other)
                        (file-name-nondirectory file-path)))
             files))
           (depth 1)
           suffix)
      (while (and (<= depth (length target-dir)) (not suffix))
        (let ((candidate
               (mapconcat #'identity
                          (last target-dir depth)
                          "/")))
          (if (seq-every-p
               (lambda (other)
                 (or (string= other file-path)
                     (not (string=
                           candidate
                           (mapconcat
                            #'identity
                            (last
                             (split-string
                              (directory-file-name
                               (file-name-directory
                                (abbreviate-file-name other)))
                              "/" t)
                             depth)
                            "/")))))
               same-name-files)
              (setq suffix candidate)
            (setq depth (1+ depth)))))
      (or suffix
          (directory-file-name
           (file-name-directory (abbreviate-file-name file-path))))))

  ;; (defun my/recentf-display-name (file-path files duplicate-table)
  ;;   "根据 FILES 和 DUPLICATE-TABLE 为 FILE-PATH 生成简洁且可区分的显示名。"
  ;;   (let ((base-name (file-name-nondirectory file-path)))
  ;;     (if (> (gethash base-name duplicate-table 0) 1)
  ;;         (format "%s    <%s>"
  ;;                 base-name
  ;;                 (my/recentf-unique-directory-suffix file-path files))
  ;;       base-name)))
  ...
  (defun my-consult--source-recentf-items ()
    ...
    (let ((ht (consult--buffer-file-hash))
          (duplicate-table (make-hash-table :test #'equal))
          normalized-files
          file-name-handler-alist
          items)
      (dolist (file recentf-list)
        (let ((file-path (my/recentf-normalize-path file)))
          (unless (or (null file-path)
                      (my/recentf-latex-generated-p file-path)
                      (gethash file-path ht))
            (push file-path normalized-files)
            (puthash (file-name-nondirectory file-path)
                     (1+ (gethash (file-name-nondirectory file-path)
                                  duplicate-table 0))
                     duplicate-table))))
      (dolist (file recentf-list)
        ...
        (let ((file-path (my/recentf-normalize-path file)))
          ...
          (unless (or (null file-path)
                      (my/recentf-latex-generated-p file-path)
                      (gethash file-path ht))
            (push (cons (my/recentf-display-name
                         file-path
                         normalized-files
                         duplicate-table)
                        file-path)
...
(use-package magit
  ...
  :preface
  (defun my/magit-fit-window (window)
    "Fit WINDOW to its Magit contents."
    (when (window-live-p window)
      (fit-window-to-buffer window nil 5)))
  (defun my/magit-display-buffer-fit-window (buffer)
    "Display Magit BUFFER below the selected window and fit its height."
    (let ((window (display-buffer
                   buffer
                   '((display-buffer-reuse-window display-buffer-below-selected)
                     (inhibit-same-window . t)
                     (window-height . fit-window-to-buffer)))))
      (my/magit-fit-window window)
      window))
  (defun my/magit-fit-status-window ()
    "Fit the visible Magit status window after its contents refresh."
    (when (derived-mode-p 'magit-status-mode)
      (when-let ((window (get-buffer-window (current-buffer) t)))
        (my/magit-fit-window window))))
  (defun my/magit-refresh-vc-mode-lines ()
    "Refresh VC mode-line state for file buffers in the current Git repository."
      ...
      (dolist (buffer (buffer-list))
        (with-current-buffer buffer
          (when (and buffer-file-name
            ...
            (vc-refresh-state)
            (force-mode-line-update))))))
  :hook ((magit-refresh-buffer . my/magit-fit-status-window)
         (magit-post-refresh . my/magit-refresh-vc-mode-lines))
  :custom
  (magit-display-buffer-function #'my/magit-display-buffer-fit-window))
#+end_src
...
(use-package treemacs
  ...
  (setq treemacs-collapse-dirs                   (if treemacs-python-executable 3 0)
        ...
        treemacs-read-string-input               (if (display-graphic-p)
                                                     'from-child-frame
                                                   'from-minibuffer)
...
Corfu handles the popup when `corfu-mode' is active; otherwise this falls back
to the configured Emacs completion UI."
...
Priority:
1. Indent current line.
2. Expand a yasnippet or jump to its next field.
3. In math, jump through `{...}' arguments and finally leave math.
4. Complete at point with Corfu/completion-at-point."
  (interactive)
...
The result is (RIGHT-POS RIGHT-LEN), where RIGHT-POS is the match end."
...
The result is (LEFT-POS LEFT-LEN), where LEFT-POS is the match start."
...
(defun my/TeX-font-completing-read (replace)
  ...
  (let* ((candidates
          (mapcar (lambda (entry)
                    (cons (string-trim (TeX-describe-font-entry entry))
                          (car entry)))
                  TeX-font-list))
...
(defun my/latex-provided-style-hook-names ()
  "Return AUCTeX style hooks recorded for the current LaTeX buffer."
  (let ((names (append (mapcar #'car LaTeX-provided-class-options)
                       (mapcar #'car LaTeX-provided-package-options))))
    ;; AUCTeX's `ctexbeamer' style hook does not pull in every Beamer
    ;; environment template, but the document class does provide them.
    (when (member "ctexbeamer" names)
      (push "beamer" names))
    (delete-dups (delq nil names))))

(defun my/latex-load-provided-style-hooks (&optional skip-update)
  "Load AUCTeX class/package style hooks for the current LaTeX buffer.
When SKIP-UPDATE is nil, refresh AUCTeX's parsed style data first."
  (when (derived-mode-p 'latex-mode 'LaTeX-mode)
    (unless skip-update
      (TeX-update-style))
    (dolist (style (my/latex-provided-style-hook-names))
      (when (and (stringp style)
                 (not (member style TeX-active-styles)))
        (ignore-errors
...
#+begin_src emacs-lisp :tangle no
;; 旧版自动引擎实现曾在这里手写解析/重写 Local Variables 块。
;; 下面的 `my/auctex-auto-engine' 也保留为候选实现，当前没有 tangle，
;; 也没有挂到 LaTeX mode 或编译命令上。
#+end_src

#+begin_src emacs-lisp :tangle no
  (defconst my/auctex-chinese-source-regexp
    (concat
     "^[[:space:]]*\\\\documentclass"
     "\\(?:\\[[^]]*\\]\\)?"
     ...
     "\\(?:\\[[^]]*\\]\\)?"
     "{\\([^}]*,\\)?ctex\\(,[^}]*\\)?}")
    "Regexp used to detect Chinese TeX support from LaTeX source.")

  (defun my/auctex-buffer-contains-regexp-p (regexp)
    "Return non-nil when current buffer contains REGEXP."
    (save-excursion
      (save-restriction
        (widen)
        (goto-char (point-min))
  ...
  (defun my/auctex-file-contains-regexp-p (file regexp)
  ...
  (defun my/auctex-buffer-uses-chinese-source-p ()
    "检查当前 buffer 源码中是否直接使用中文 LaTeX 支持。"
    (my/auctex-buffer-contains-regexp-p my/auctex-chinese-source-regexp))

  (defun my/auctex-master-file-path ()
    "返回当前 LaTeX 缓冲区对应 master 文件的绝对路径。"
    (when-let ((buffer-file (buffer-file-name))
               (buffer-dir (file-name-directory buffer-file)))
      (cond
       ((and (boundp 'TeX-master)
             (stringp TeX-master))
        (expand-file-name TeX-master buffer-dir))
       ((fboundp 'TeX-master-file)
        (when-let ((master (ignore-errors (TeX-master-file t))))
          (when (stringp master)
            (expand-file-name master buffer-dir)))))))

  (defun my/auctex-master-uses-chinese-source-p ()
    "检查 master 文件源码中是否使用中文 LaTeX 支持。"
    (let ((master-file (my/auctex-master-file-path))
          (buffer-file (buffer-file-name)))
      (and master-file
           (not (and buffer-file
                     (ignore-errors (file-equal-p master-file buffer-file))))
           (my/auctex-file-contains-regexp-p
            master-file
            my/auctex-chinese-source-regexp))))

  (defun my/auctex-uses-chinese-p ()
    "通过当前 buffer 和 master 源码判断是否需要中文 TeX 引擎。"
    (or (my/auctex-buffer-uses-chinese-source-p)
        (my/auctex-master-uses-chinese-source-p)))

  (defun my/auctex-current-file-local-engine ()
    "返回当前文件中声明的 `TeX-engine'，未声明时返回 nil。"
    (cdr (assq 'TeX-engine file-local-variables-alist)))

  (defun my/auctex-write-engine-to-file (engine)
    "使用 Emacs 内置接口把 ENGINE 写入文件局部变量。"
    (when (buffer-file-name)
      (let ((current (my/auctex-current-file-local-engine)))
        (unless (eq current engine)
          (add-file-local-variable 'TeX-engine engine)
          (setf (alist-get 'TeX-engine file-local-variables-alist) engine)))))

  (defun my/auctex-auto-engine (&optional write-file-local)
    "根据文档内容自动设置 `TeX-engine'。
若 WRITE-FILE-LOCAL 非 nil，则同步到文件局部变量。"
    (when (derived-mode-p 'latex-mode 'LaTeX-mode)
      (let ((engine (if (my/auctex-uses-chinese-p) 'xetex 'luatex)))
        (unless (and (boundp 'TeX-engine)
                     (eq TeX-engine engine))
          (setq-local TeX-engine engine))
        (when write-file-local
          (my/auctex-write-engine-to-file engine)))))
#+end_src
...
(use-package tex
  ...
  (setq TeX-style-private my/tex-el-dir
        TeX-auto-private my/tex-el-dir)
  ...
  ;; 在编译命令前自动触发
  ;; (advice-add 'TeX-command-master :before (lambda (&rest _) (my/auctex-auto-engine t)))
  ;; (advice-add 'TeX-command-run-all :before (lambda (&rest _) (my/auctex-auto-engine t)))

  (defun my/latex-mode-setup ()
    "Custom setup for LaTeX mode."
    (interactive)
    ;; (my/auctex-auto-engine)                  ; 新版引擎检测
    (setq-local TeX-electric-math (cons "\\("  "\\)"))
    (my/latex-keymap-setup)
    (my/latex-load-provided-style-hooks)
