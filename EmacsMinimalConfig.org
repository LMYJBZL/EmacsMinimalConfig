...
#+AUTHOR: Baojie
#+DATE: Time-stamp: <2026-04-26 15:14:22 baojie>
:PROPERTIES:
#+STARTUP: content
...
(let ((path-hash (make-hash-table :test 'equal))
      (prefix-list nil))
  (dolist (p my/recentf-keep-paths)
    (if (directory-name-p p)
        (push (file-name-as-directory (expand-file-name p)) prefix-list)
      (puthash (expand-file-name p) t path-hash)))
  (setq recentf-keep (list (lambda (file)
                             (let ((full (expand-file-name file)))
                               (or (gethash full path-hash)
                                   (cl-some (lambda (dir) (string-prefix-p dir full))
                                            prefix-list)))))))
  )
#+end_src
