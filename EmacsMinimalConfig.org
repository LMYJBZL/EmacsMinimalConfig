...
#+AUTHOR: Baojie
#+DATE: Time-stamp: <2026-04-27 12:35:05 baojie>
:PROPERTIES:
#+STARTUP: content
...
(defun my/tex-help-fixed-width ()
  (let ((help-win (get-buffer-window "*TeX Help*")))
    (when help-win
      (let ((current (window-total-width help-win))
            (target 76))
        (window-resize help-win (- target current) t))))) ;; t = 横向调整
    ...
    (let ((ht (consult--buffer-file-hash))
      ...
      (dolist (file recentf-list)
        ...
        (let ((file-path (my/recentf-normalize-path file)))
          ;; 跳过已在缓冲区的文件
          (unless (or (null file-path) (gethash file-path ht))
            (push (propertize (my/recentf-display-name
                               file-path
                               normalized-files
                               duplicate-table)
...
(defun my-auctex--local-vars-start-regexp ()
  "Return the regexp matching the Local Variables line."
  (format "^[[:space:]]*%s[[:space:]]*Local Variables:[[:space:]]*$"
          (regexp-quote my-auctex-auto-engine-local-vars-prefix)))

(defun my-auctex--local-vars-end-regexp ()
  "Return the regexp matching the End line."
  (format "^[[:space:]]*%s[[:space:]]*End:[[:space:]]*$"
          (regexp-quote my-auctex-auto-engine-local-vars-prefix)))

(defun my-auctex--local-vars-entry-regexp ()
  "Return the regexp matching one local variable entry line."
...
(defun my-auctex--file-local-variables-bounds ()
  ...
  (save-excursion
    (goto-char (point-max))
    (let (start end)
      (when (re-search-backward (my-auctex--local-vars-start-regexp) nil t)
        (setq start (line-beginning-position))
        (when (re-search-forward (my-auctex--local-vars-end-regexp) nil t)
...
(defun my-auctex--inspect-local-vars-block ()
  "Inspect the current file-local variables block.

Return a plist with keys:
`:bounds'    bounds of the block or nil
`:variables' alist of valid variables found in the block
`:valid'     non-nil when the block contains no blank or malformed lines."
  (let ((bounds (my-auctex--file-local-variables-bounds))
        (entry-regexp (my-auctex--local-vars-entry-regexp))
        variables
        (valid t))
    (when bounds
      (save-excursion
        ...
        (while (< (point) (cdr bounds))
          (let ((line (string-trim-right
            ...
            (cond
             ((string-blank-p line)
              (setq valid nil))
             ((string-match entry-regexp line)
              (let ((name (match-string 1 line))
                    (value (match-string 2 line)))
                (unless (member name '("Local Variables" "End"))
                  (push (cons (intern name) value) variables))))
             (t
              (setq valid nil))))
          (forward-line 1))))
    (list :bounds bounds
          :variables (nreverse variables)
          :valid (and bounds valid))))

(defun my-auctex--current-local-vars-engine (variables)
  "Return the current file-local `TeX-engine' value from VARIABLES, or nil."
  (cdr (assq 'TeX-engine variables)))

(defun my-auctex--local-vars-block (engine &optional variables)
  "Return a normalized file-local variable block for ENGINE.
VARIABLES should be an alist of existing local variables to preserve."
  (let* ((variables (assq-delete-all 'TeX-engine (copy-sequence variables)))
         (variables (append variables
                            (list (cons 'TeX-engine (symbol-name engine))))))
    (mapconcat
     #'identity
     (append
      (list (format "%s Local Variables:" my-auctex-auto-engine-local-vars-prefix))
      (mapcar
       (lambda (entry)
         (format "%s %s: %s"
                 my-auctex-auto-engine-local-vars-prefix
                 (car entry)
                 (cdr entry)))
       variables)
      (list (format "%s End:" my-auctex-auto-engine-local-vars-prefix)))
     "\n")))

(defun my-auctex--write-engine-to-local-variables (engine &optional variables bounds)
  "Write ENGINE into a normalized file-local variable block.
VARIABLES and BOUNDS should come from `my-auctex--inspect-local-vars-block'."
  (let ((block (my-auctex--local-vars-block engine variables)))
    (save-excursion
      (save-restriction
        (widen)
        (if bounds
            (let ((start (car bounds))
                  (end (cdr bounds)))
              ;; Eat one trailing newline after the old block if it exists,
              ;; so repeated rewrites do not accumulate blank lines.
...
(defun my-auctex-apply-auto-engine ()
  ...
  (when (derived-mode-p 'tex-mode 'plain-tex-mode 'latex-mode 'LaTeX-mode)
    (let* ((engine (my-auctex--desired-engine))
           (info (my-auctex--inspect-local-vars-block))
           (bounds (plist-get info :bounds))
           (variables (plist-get info :variables))
           (valid (plist-get info :valid))
           (current (my-auctex--current-local-vars-engine variables)))
      (when (or (not valid)
                (not (equal current (symbol-name engine))))
        (my-auctex--write-engine-to-local-variables engine variables bounds)
...
#+begin_src emacs-lisp
  (defconst my/auctex-chinese-styles
    '("ctex" "ctexart" "ctexrep" "ctexbook" "ctexbeamer")
    "Style names that imply a Chinese TeX engine is required.")
  ...
  (defconst my/auctex-chinese-source-regexp
    (concat
     ...
     "\\(?:\\[[^]]*\\]\\)?"
     "{\\([^}]*,\\)?ctex\\(,[^}]*\\)?}")
    "Regexp used as a fallback when AUCTeX style data is unavailable.")

  (defconst my/auctex-chinese-style-el-regexp
    "\"\\(ctex\\|ctexart\\|ctexrep\\|ctexbook\\|ctexbeamer\\)\""
    "Regexp used to inspect generated AUCTeX style files for Chinese support.")

  (defun my/auctex-file-uses-chinese-source-p (file)
    "检查 FILE 源码中是否直接使用中文 LaTeX 支持。"
    (when (and file (file-readable-p file))
      (with-temp-buffer
        (insert-file-contents file)
        (goto-char (point-min))
        (re-search-forward my/auctex-chinese-source-regexp nil t))))

  (defun my/auctex-buffer-uses-chinese-source-p ()
    "回退到源码检测当前缓冲区是否使用中文 LaTeX 支持。"
    (or (my/auctex-file-uses-chinese-source-p (buffer-file-name))
        (save-excursion
          (goto-char (point-min))
          (re-search-forward my/auctex-chinese-source-regexp nil t))))
  ...
  (defun my/auctex-master-uses-chinese-source-p ()
    ...
    (let ((master-file (my/auctex-master-file-path))
      ...
      (and master-file
           (not (and buffer-file
                     (file-equal-p master-file buffer-file)))
           (my/auctex-file-uses-chinese-source-p master-file))))

  (defun my/auctex-style-search-dirs ()
    "返回应当检查的 AUCTeX 样式目录列表。"
    (delete-dups
     (delq nil
           (append
            (when-let ((buffer-file (buffer-file-name))
                       (buffer-dir (file-name-directory buffer-file))
                       (auto-local (and (boundp 'TeX-auto-local) TeX-auto-local)))
              (list (expand-file-name auto-local buffer-dir)))
            (when-let ((master-file (my/auctex-master-file-path))
                       (master-dir (file-name-directory master-file))
                       (auto-local (and (boundp 'TeX-auto-local) TeX-auto-local)))
              (list (expand-file-name auto-local master-dir)))
            (when (and (boundp 'TeX-style-private)
                       (stringp TeX-style-private))
              (list (expand-file-name TeX-style-private)))
            (when (and (boundp 'TeX-auto-private)
                       (stringp TeX-auto-private))
              (list (expand-file-name TeX-auto-private)))))))

  (defun my/auctex-style-file-uses-chinese-p (style)
    "检查 STYLE 对应的 AUCTeX style 文件是否间接启用了中文支持。"
    (cl-some
     (lambda (dir)
       (let ((style-file (expand-file-name (format "%s.el" style) dir)))
         (when (file-readable-p style-file)
           (with-temp-buffer
             (insert-file-contents style-file)
             (goto-char (point-min))
             (re-search-forward my/auctex-chinese-style-el-regexp nil t)))))
     (my/auctex-style-search-dirs)))

  (defun my/auctex-style-files-use-chinese-p ()
    "检查当前激活样式对应的 el 文件是否间接启用了中文支持。"
    (and (boundp 'TeX-active-styles)
         (cl-some #'my/auctex-style-file-uses-chinese-p TeX-active-styles)))

  (defun my/auctex-uses-chinese-p ()
    "综合当前文件、master 文件和 AUCTeX 样式信息判断中文支持。"
    (or (and (boundp 'TeX-active-styles)
             (cl-some (lambda (style)
                        (member style TeX-active-styles))
                      my/auctex-chinese-styles))
        (my/auctex-buffer-uses-chinese-source-p)
        (my/auctex-master-uses-chinese-source-p)
        (my/auctex-style-files-use-chinese-p)))
  ...
  (defun my/auctex-auto-engine ()
    "根据文档内容自动设置 `TeX-engine'，并同步到文件局部变量。"
...
(use-package tex
  ...
  ;; 在编译命令前自动触发
  (advice-add 'TeX-command-master :before (lambda (&rest _) (my/auctex-auto-engine)))
  (advice-add 'TeX-command-run-all :before (lambda (&rest _) (my/auctex-auto-engine)))

  ;; (advice-add 'TeX-normal-mode :after #'my-auctex--apply-auto-engine-before) 
