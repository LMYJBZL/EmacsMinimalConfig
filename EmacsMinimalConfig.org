...
#+AUTHOR: Baojie
#+DATE: Time-stamp: <2026-07-06 20:26:41 baojie>
:PROPERTIES:
#+STARTUP: content
...
#+begin_src emacs-lisp
(defcustom my/minibuffer-completion-frontend 'vertico
  "Minibuffer completion frontend.

Use `vertico' for Vertico and its category-aware multiform display.
Use `default' to fall back to the built-in Emacs completion UI.  Corfu is
independent of this setting and continues to provide in-buffer completion."
  :type '(choice (const :tag "Vertico" vertico)
                 (const :tag "Built-in Emacs completion" default))
  :group 'completion)

;; These settings are only for the built-in fallback frontend.  Vertico has
;; its own display, selection and sizing controls below.
(when (eq my/minibuffer-completion-frontend 'default)
...
Vertico 只负责 minibuffer；Corfu 继续负责正文中的
=completion-at-point=。如果需要退回 Emacs 原生 minibuffer 补全，将
=my/minibuffer-completion-frontend= 改为 =default= 并重启 Emacs。

#+begin_src emacs-lisp
(use-package vertico
  :ensure t
  :if (eq my/minibuffer-completion-frontend 'vertico)
  :bind
  (("M-R" . vertico-repeat)
   :map vertico-map
   ("TAB" . vertico-insert)
  ...
  (unless (advice-member-p #'my/vertico-crm-indicator
    ...
    (advice-add #'completing-read-multiple
                :filter-args #'my/vertico-crm-indicator))
...
(defun my/graphic-frame-p (&optional frame)
  "Return non-nil when FRAME is graphical."
  (display-graphic-p frame))

(defun my/all-the-icons-dired-maybe-enable ()
  "Enable Dired icons only in graphical frames."
  (when (my/graphic-frame-p)
    (all-the-icons-dired-mode 1)))

(defun my/all-the-icons-enable-for-frame (frame)
  "Enable graphical icon UI after creating FRAME."
  (with-selected-frame frame
    (when (my/graphic-frame-p frame)
...
(use-package all-the-icons
  ...
  :defer t
  :config
  ;; Unknown files use Octicons as fallback.
  (setq all-the-icons-default-file-icon
        '(all-the-icons-octicon "file-text"
          :v-adjust 0.0
          :face all-the-icons-dsilver))

  ;; Avoid the old FontAwesome fallback for *Messages* buffers.
  (setf (alist-get 'messages-buffer-mode all-the-icons-mode-icon-alist)
...
(use-package all-the-icons-dired
  :ensure t
  :after all-the-icons
  :hook (dired-mode . my/all-the-icons-dired-maybe-enable))
...
(use-package all-the-icons-completion
  ...
  ;; Ensure directories receive directory icons even without a trailing slash.
  (cl-defmethod all-the-icons-completion-get-icon
    (cand (_cat (eql file)))
    (let* ((path (substring-no-properties cand))
           (directory-p
  ...
  ;; Keep icon column alignment stable.
  (cl-defmethod all-the-icons-completion-get-icon :around (cand cat)
    (let* ((icon (string-trim-right (cl-call-next-method)))
           (padding (max 0 (- 2 (string-width icon)))))
      (concat (make-string padding ?\s) icon " "))))
...
(use-package kind-icon
  ...
  :after (corfu svg-lib)
  :custom
  ;; Suitable for normal GUI Emacs and daemon + GUI emacsclient.
  (kind-icon-use-icons (or (display-graphic-p) (daemonp)))
...
(add-hook 'after-make-frame-functions
          #'my/all-the-icons-enable-for-frame)

(add-hook 'emacs-startup-hook
          (lambda ()
            (my/all-the-icons-enable-for-frame (selected-frame))))
#+end_src
