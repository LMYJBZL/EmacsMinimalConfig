...
#+AUTHOR: Baojie
#+DATE: Time-stamp: <2026-07-17 10:35:28 baojie>
:PROPERTIES:
#+STARTUP: content
...
(add-to-list 'display-buffer-alist
      '(("\\*TeX errors\\*"
        ...
        ;; Magit 状态窗口：在当前选中窗口下方打开，不占用侧边区域
        ((derived-mode . magit-status-mode)
         (display-buffer-reuse-window display-buffer-below-selected)
         (window-height . 0.33)
         (inhibit-same-window . nil))   ; 允许在当前窗口重用，但优先在下侧打开
        ))
...
(use-package vertico
  ...
  (setq vertico-multiform-commands
        '((consult-line reverse)
          ...
          (consult-buffer (vertico-resize . t))
          (describe-symbol
...
;; AUCTeX 14.1.2 does not attach a completion category to the dynamic
;; completion table used by `TeX-insert-macro' (`C-c C-m').  Add a dedicated
;; category so Vertico multiform can reliably select its grid layout.
(with-eval-after-load 'tex
  (defun my/tex-macro-completion-table
      (table string predicate action)
    "Run TABLE while exposing the `tex-macro' completion category."
    (if (eq action 'metadata)
        '(metadata (category . tex-macro))
      (complete-with-action action table string predicate)))

  (defun my/tex-add-macro-completion-category (table)
    "Wrap AUCTeX completion TABLE with `tex-macro' metadata."
    (apply-partially #'my/tex-macro-completion-table table))

  (unless (advice-member-p #'my/tex-add-macro-completion-category
                           #'TeX--symbol-completion-table)
    (advice-add #'TeX--symbol-completion-table
                :filter-return
                #'my/tex-add-macro-completion-category)))
#+end_src
...
(use-package magit
  ...
  :config
  (defvar my/magit-key-hints-buffer-name "*Magit Keys*")

  (defconst my/magit-key-hints
    '(("s" "stage"          "u" "unstage"        "k" "discard")
      ("c c" "commit"       "P p" "push"         "F p" "pull")
      ("b b" "checkout"     "b c" "new branch"   "m m" "merge")
      ("l l" "log"          "d d" "diff"         "g" "refresh")
      ("TAB" "section"      "RET" "visit"        "q" "quit"))
    "Compact Magit key hints shown below `magit-status'.")

  (defun my/magit-key-hints-buffer ()
    "Return a refreshed buffer containing compact Magit key hints."
    (with-current-buffer (get-buffer-create my/magit-key-hints-buffer-name)
      (let ((inhibit-read-only t))
        (erase-buffer)
        (insert "Magit keys\n\n")
        (dolist (row my/magit-key-hints)
          (pcase-let ((`(,k1 ,d1 ,k2 ,d2 ,k3 ,d3) row))
            (insert (format "%-4s %-12s  %-4s %-12s  %-4s %s\n"
                            k1 d1 k2 d2 k3 d3)))))
      (special-mode)
  ...
  (defun my/magit-show-key-hints ()
    "Show compact Magit key hints below the status window."
    (display-buffer
     (my/magit-key-hints-buffer)
     '((display-buffer-reuse-window display-buffer-below-selected)
       (window-height . fit-window-to-buffer)
       (dedicated . t))))

  (add-hook 'magit-status-mode-hook #'my/magit-show-key-hints))
#+end_src
