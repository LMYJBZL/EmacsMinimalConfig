...
#+AUTHOR: Baojie
#+DATE: Time-stamp: <2026-04-16 21:48:09 baojie>
:PROPERTIES:
#+STARTUP: content
...
(use-package recentf
  ...
  (defconst my/recentf-keep-dirs-expanded
    (mapcar #'expand-file-name my/recentf-keep-dirs)
    "Expanded paths for faster lookup")

  (setq recentf-keep (list 'recentf-keep-default-predicate
                           (lambda (file)
                             (let ((full-path (expand-file-name file)))
                               (cl-some (lambda (dir) (string-prefix-p dir full-path))
                                        my/recentf-keep-dirs-expanded))))))
#+end_src
