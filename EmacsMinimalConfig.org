...
;; 编译后将 PDF 从 `TeX-output-dir' 复制到 master 文件所在目录。
(defun my/auctex-copy-pdf-to-master-dir (output-filename)
  "Copy compiled PDF OUTPUT-FILENAME beside the master TeX file."
  (when (and (string-equal (file-name-extension output-filename) "pdf")
             (file-exists-p output-filename))
    (let* ((source (expand-file-name output-filename))
           (master-buffer (and (boundp 'TeX-command-buffer)
